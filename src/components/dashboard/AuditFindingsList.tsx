...
//   const isCanCreateFinding = user.role === ('IT_Auditors' ) || user.role === 'Inspection_Auditors' || user.role === 'Resident_Auditors';
//   const isCanEditFinding = user.role === 'IT_Auditors'  || user.role === 'Inspection_Auditors' || user.role === 'Resident_Auditors' || user.role === 'Audit_Supervisor' || user.role === 'IT_Auditors_Director'  || user.role === 'IT_Auditors_Supervisor' || user.role === 'Inspection_Auditors_Supervisor';
//   const isCanDeleteFinding = user.role === 'IT_Auditors'  || user.role === 'Inspection_Auditors' || user.role === 'Resident_Auditors' || user.role === 'Audit_Supervisor' || user.role === 'IT_Auditors_Director'  || user.role === 'IT_Auditors_Supervisor' || user.role === 'Inspection_Auditors_Supervisor';
//   const isCanAssignment = user.role === 'Auditee_Supervisor' || user.role === 'Inspection_Auditees_Supervisor'  || user.role === 'IT_Auditees_Supervisor';
...
//       const response = await api.get(`ZAMS/api/audit-findings?${params.toString()}`);
...
//             {(onUnifiedCreate && isCanCreateFinding) && (
//               <div className="flex items-center gap-3">
...
export const AuditFindingsList = ({
  ...
  // Apply initial filter when component mounts or when initialFilter changes

  useEffect(() => {

    if (initialFilter) {

      const newFilters: AuditFindingFilters = {
      ...
      // If riskRating filter is provided, we need to find the corresponding risk_rating_id

      if (initialFilter.riskRating) {

        // We'll apply the risk rating filter after the reference data is loaded

        // For now, just fetch with status filter
  ...
  // Apply risk rating filter after reference data is loaded

  useEffect(() => {

    if (initialFilter?.riskRating && riskRatings.length > 0) {

      const riskRating = riskRatings.find(

        r => r.risk_rating_name === initialFilter.riskRating ||

             r.risk_rating_name === 'High' && initialFilter.riskRating === 'High' ||

             r.risk_rating_name === 'Critical' && initialFilter.riskRating === 'High'
  ...
  }, [riskRatings, initialFilter?.riskRating]);
